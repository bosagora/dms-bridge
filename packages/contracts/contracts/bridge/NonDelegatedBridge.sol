...
import "./NonDelegatedBridgeStorage.sol";

import "../lib/BridgeLib.sol";

contract NonDelegatedBridge is
    NonDelegatedBridgeStorage,
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    IBridge,
    IBridgeLiquidity
{
    ...
    function depositToBridge(
        ...
        if (tokenInfos[_tokenId].native) {
            ...
            ERC20 token = tokenInfos[_tokenId].token;
            require(_amount <= token.allowance(_account, address(this)), "1515");
            if (token.transferFrom(_account, address(this), _amount)) {
    ...
    function executeWithdraw(
        ...
        if (!withdraws[_withdrawId].executed && _isConfirmed(_withdrawId)) {
            ...
            if (tokenInfos[tokenId].native) {
                ...
                ERC20 token = tokenInfos[tokenId].token;
                if (token.balanceOf(address(this)) >= withdraws[_withdrawId].amount) {
    ...
    function depositLiquidity(
        ...
        if (tokenInfos[_tokenId].native) {
            ...
            require(_amount <= token.allowance(_msgSender(), address(this)), "1515");
