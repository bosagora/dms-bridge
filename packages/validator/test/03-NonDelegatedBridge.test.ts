...
import { Deployments } from "./helper/NonDelegatedDeployments";
...
describe("Test for NonDelegatedBridge", () => {
    ...
    let tokenAContract: TestERC20;
    let tokenBContract: TestERC20;
    ...
    before("Deploy", async () => {
        ...
        tokenAContract = deploymentsA.getContract("TestERC20") as TestERC20;
        tokenBContract = deploymentsB.getContract("TestERC20") as TestERC20;
    ...
    before("Deposit BIP20 Liquidity at Bridge A", async () => {
        await hre.changeNetwork(config.bridge.networkAName);
        const liquidityAmount = Amount.make(1_000_000_000, 18).value;
        await tokenAContract.connect(deploymentsA.accounts.deployer).approve(bridgeAContract.address, liquidityAmount);
        const tx1 = await bridgeAContract
            .connect(deploymentsA.accounts.deployer)
    ...
    before("Deposit BIP20 Liquidity at Bridge B", async () => {
        await hre.changeNetwork(config.bridge.networkBName);
        const liquidityAmount = Amount.make(1_000_000_000, 18).value;
        await tokenBContract.connect(deploymentsB.accounts.deployer).approve(bridgeBContract.address, liquidityAmount);
        const tx1 = await bridgeBContract
            .connect(deploymentsB.accounts.deployer)
            .depositLiquidity(tokenId1, liquidityAmount, 0, HashZero);
        await tx1.wait();
    ...
    it("Deposit BIB20 token to Main Bridge", async () => {
        ...
        await tokenBContract.connect(deploymentsB.accounts.users[0]).approve(bridgeBContract.address, amount);
        await expect(
            bridgeBContract
                .connect(deploymentsB.accounts.deployer)
